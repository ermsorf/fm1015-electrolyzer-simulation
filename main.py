...
for step in range(steps):
    try:
        system.step(dt)
        time_history.append(system.time / 60)
        ...
        custom_property_history["H2_O2_fraction"].append(system.anode.mols["GH2"] / system.anode.mols["GO2"])


        custom_property_history["Anode Volume Fraction"].append(system.anode.liquid_volume / system.anode.volume)
        # print("anode liquid volume: ",system.anode.liquid_volume)
        # print("anode volume: ", system.anode.volume)
...
ax1.plot(time_history, tanks_mol_history["anode"]["LH2O"], label=r"$x_{H_2O}$", linewidth=2, color=COLOR_H2O)
ax1.set_xlabel("Time (min)")
ax1.set_ylabel(r"$x_{H_2O}/ mol$", fontsize=16)
ax1.legend(loc='center right')
ax1.grid(True)

ax2.plot(time_history, tanks_mol_history["anode"]["LH2"], label=r"$x_{H_2}$", linewidth=1.5, color=COLOR_H2)
ax2.plot(time_history, tanks_mol_history["anode"]["LO2"], label=r"$x_{O_2}$", linewidth=1.5, color=COLOR_O2)
ax2.set_xlabel("Time/ min")
ax2.set_ylabel(r"$x_{H_2} / x_{O_2}$ (mol)")
...
ax1.plot(time_history, tanks_mol_history["anode"]["GO2"], label=r"y_{O_2}", linewidth=2, color=COLOR_O2)
ax1.set_xlabel("Time (s)")
ax1.set_ylabel("GO2 (mol)")
ax1.legend(loc='center right')
ax1.grid(True)
...
plt.plot(time_history, [cathode_drag[i]['LO2'] for i in range(len(time_history))], label="Drag", linewidth=2, color='orange')
...
fig, (ax1, ax2) = plt.subplots(1,2,figsize=(8, 4))
ax1.plot(time_history, [custom_property_history["Cathode Pressure"][i] / 1e5 for i in range(len(custom_property_history["Cathode Pressure"]))], linewidth=2, color='green')
...
ax2.plot(time_history, [custom_property_history["Anode Pressure"][i] / 1e5 for i in range(len(custom_property_history["Anode Pressure"]))], linewidth=2, color='red')
...
plt.figure(figsize=(8, 4))
plt.plot(time_history, custom_property_history["Anode Volume Fraction"], linewidth=2, color='blue', label=r'$V_{a}^{l} / V_{a}$')
plt.plot(time_history, custom_property_history["Cathode Volume Fraction"], linewidth=2, color='cyan', label=r'$V_{c}^{l} / V_{c}$')
